...
# Нахлёст назад от последней посчитанной даты. Microsoft Planetary Computer
# публикует L2A с задержкой в несколько дней и не строго по порядку: снимок за
# 28-е может появиться раньше, чем за 26-е. Широкое окно гарантирует, что
# «опоздавшая» дата не окажется левее границы поиска и не потеряется. Стоит это
# ничего: даты, уже присутствующие в данных, отбрасываются без расчёта.
LOOKBACK = 14
SEASON_START = "-03-01"
SEASON_END = "-11-01"
